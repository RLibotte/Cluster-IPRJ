...
void sweep_1(int M, int G, int nxf, int nyf, vector<double> mi, vector<double> n, vector<double> w,
	...
	for(int i = nyf - 1 ; i >= 0 ; i--){
		for(int j = 0 ; j < nxf ; j++){
		    for(int g = 0 ; g < G ; ++g){
				for(int k = 0 ; k < M/4 ; k++){
					psi_x_b[i][j][k + g*M] = (2 * fabs(mi[k]) * psi_x[i][j][k + g*M]/h_x_nodes[i][j] + 2 * fabs(n[k]) * psi_y[i+1][j][k + g*M]/h_y_nodes[i][j] + 
										SS_x[i][j][g] + SS_x_1[i][j][g][k] + q_nodes[i][j][g]) / 
										(2 * fabs(mi[k])/h_x_nodes[i][j] + 2 * fabs(n[k])/h_y_nodes[i][j] + sigmat_z[zcn[i][j]][g]);

					psi_y_b[i][j][k + g*M] = (2 * fabs(mi[k]) * psi_x[i][j][k + g*M]/h_x_nodes[i][j] + 2 * fabs(n[k]) * psi_y[i+1][j][k + g*M]/h_y_nodes[i][j] + 
										SS_y[i][j][g] + SS_y_1[i][j][g][k] + q_nodes[i][j][g]) / 
										(2 * fabs(mi[k])/h_x_nodes[i][j] + 2 * fabs(n[k])/h_y_nodes[i][j] + sigmat_z[zcn[i][j]][g]);

					psi_x[i][j+1][k + g*M] = 2 * psi_x_b[i][j][k + g*M] - psi_x[i][j][k + g*M];
					psi_y[i][j][k + g*M] = 2 * psi_y_b[i][j][k + g*M] - psi_y[i+1][j][k + g*M];
