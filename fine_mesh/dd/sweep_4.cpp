...
void sweep_4(int M, int G, int nxf, int nyf, vector<double> mi, vector<double> n, vector<double> w,
		...
		for(int j = 0 ; j < nxf ; j++){
			for(int g = 0 ; g < G ; ++g){
				for(int k = 3 * M/4 ; k < M ; k++){
					psi_x_b[i][j][k + g*M] = (2 * fabs(mi[k]) * psi_x[i][j][k + g*M]/h_x_nodes[i][j] + 2 * fabs(n[k]) * psi_y[i][j][k + g*M]/h_y_nodes[i][j] + 
										SS_x[i][j][g] + SS_x_1[i][j][g][k] + q_nodes[i][j][g]) / 
										(2 * fabs(mi[k])/h_x_nodes[i][j] + 2 * fabs(n[k])/h_y_nodes[i][j] + sigmat_z[zcn[i][j]][g]);

					psi_y_b[i][j][k + g*M] = (2 * fabs(mi[k]) * psi_x[i][j][k + g*M]/h_x_nodes[i][j] + 2 * fabs(n[k]) * psi_y[i][j][k + g*M]/h_y_nodes[i][j] + 
