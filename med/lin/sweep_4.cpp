...
void sweep_4(const int &M, 
	...
	int MG = M*G, index;
		...
		for(int i = 0 ; i < nyf ; i++){
			...
			quad_l_x(j, i, M, G, nxf, nyf, mi, n, zcn, A_p_inv_z, l_y, q, h_x, h_y, c1_x, b0_x, b1_x, psi_x, psi_y, M/2, 3*M/4, mhx, nhy);
			quad_l_y(j, i, M, G, nxf, nyf, mi, n, zcn, A_p_inv_z, l_x, q, h_x, h_y, c1_y, b0_y, b1_y, psi_x, psi_y, M/2, 3*M/4, mhx, nhy);

			for(int g = 0 ; g < G ; g++){
				for(int k = 0 ; k < M/4 ; k++){

					index = k + g*M;
					b_in_x[index] = psi_x[i][j][index]     - (b0_x[i][j][index] - b1_x[i][j][index]);
					b_in_y[index] = psi_y[i + 1][j][index] - (b0_y[i][j][index] - b1_y[i][j][index]);

					index += M/4;
					b_in_x[index] = psi_x[i][j + 1][index] - (b0_x[i][j][index] + b1_x[i][j][index]);
					b_in_y[index] = psi_y[i + 1][j][index] - (b0_y[i][j][index] - b1_y[i][j][index]);
			
					index += M/4;
					b_in_x[index] = psi_x[i][j + 1][index] - (b0_x[i][j][index] + b1_x[i][j][index]);
					b_in_y[index] = psi_y[i][j][index]     - (b0_y[i][j][index] + b1_y[i][j][index]);
			...
			for(int g = 0 ; g < G ; g++){
				for(int k = 0 ; k < M ; k++){
					index = k + g*M;
					psi_x_p_out[i][j][k] = (b0_x[i][j][index]);// + b1_x[i][j][index] - b2_x[i][j][index]);
			...
			for(int g = 0 ; g < G ; g++){
				for(int k = 3*M/4 ; k < M ; k++){
					...
					psi_x[i][j+1][index] = psi_x_out[index] + (b0_x[i][j][index] + b1_x[i][j][index]);
