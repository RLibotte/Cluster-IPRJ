...
void sweep_1(const int &M, 
	...
	for(int i = nyf - 1 ; i >= 0 ; i--){
		for(int j = 0 ; j < nxf ; j++){
			...
			quad_l_x(j, i, M, G, nxf, nyf, mi, n, zcn, A_p_inv_z, l_y, q, h_x, h_y, c1_x, c2_x, b0_x, b1_x, b2_x, psi_x, psi_y, mhx, nhy, 3*M/4, M);
			quad_l_y(j, i, M, G, nxf, nyf, mi, n, zcn, A_p_inv_z, l_x, q, h_x, h_y, c1_y, c2_y, b0_y, b1_y, b2_y, psi_x, psi_y, mhx, nhy, 3*M/4, M);
			...
			for(int g = 0 ; g < G ; g++){
				for(int k = 0 ; k < M/4 ; k++){
					...
					psi_x[i][j+1][index] = psi_x_out[index] + (b0_x[i][j][index] + b1_x[i][j][index] - b2_x[i][j][index]);
					psi_y[i][j][index] = psi_y_out[index]   + (b0_y[i][j][index] + b1_y[i][j][index] - b2_y[i][j][index]);
