...
#include "../../leakage/get_quad_l.cpp"
...
double med_quad(string model, const int N, const int M, const double max_dev, const int wx, const int wy){
	...
	do{ ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		sweep_1(M, G, nxf, nyf, l_x, l_y, mi, n, w, psi_x, psi_y, psi_x_old, psi_y_old, l_x_temp, l_y_temp, 
				...
				c1_x, c2_x, c1_y, c2_y, b0_x, b1_x, b2_x, b0_y, b1_y, b2_y, zcn, A_p_inv, psi_in_x_inv_arr, psi_in_y_inv_arr, mhx, nhy);
...
int main(){
	...
	for(int i = 0 ; i < wx_vec.size() ; i++){
		...
		for(int j = 0 ; j < iter ; j++){
			time += med_quad(model, N, M, max_dev, wx_vec[i], wx_vec[i]);
		}
		printf(" %6f \n\n", time/iter);
	}

	// for(int i = 0 ; i < wx_vec.size() ; i++){
	// 	med_quad(model, N, M, max_dev, wx_vec[i], wx_vec[i]);
	// }
