...
void sweep_3(const int &M, 
	...
	int MG = M*G, index;
		...
		for(int i = 0 ; i < nyf ; i++){
			...
			mult_part(psi_iter_x[i][j], b_in_x, b_in_x, M/2, 3*M/4, G, M);
			mult_part(psi_aux_x[i][j], l_y_temp, l_y_temp, M/2, 3*M/4, G, M);

			mult_part(psi_iter_y[i][j], b_in_y, b_in_y, M/2, 3*M/4, G, M);
			mult_part(psi_aux_y[i][j], l_x_temp, l_x_temp, M/2, 3*M/4, G, M);
