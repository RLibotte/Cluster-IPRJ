...
double rm_lin(string model, const int N, const int M, const double max_dev, const int wx, const int wy){
	...
	auto [b_in_x, b_in_y, psi_x_p, psi_y_p, l_x_temp, l_y_temp, psi_x_out, psi_y_out, 
		  ...
		  psi_x_old, psi_y_old, psi_in_x_inv_arr, psi_in_y_inv_arr, c1_x, b0_x, b1_x, c1_y, b0_y, b1_y, mhx, nhy, A_p_lu,
	...
	get_psi_in_inv(nxf, nyf, M, G,  D_x, D_y, V_x, V_y, h_x, h_y, psi_in_x_inv, psi_in_y_inv, zone_config, zcn, psi_in_x_inv_arr, psi_in_y_inv_arr);
	...
	do{ ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		sweep_1(M, G, nxf, nyf, l_x, l_y, mi, n, w, psi_x, psi_y,
				...
				c1_x, c1_y, b0_x, b1_x, b0_y, b1_y, 
				zcn, A_p_inv, mhx, nhy, psi_x_aux, psi_y_aux);
...
int main(){
	...
	// rm_lin(model, N, M, max_dev, wx, wy);
	...
	for(int i = 0 ; i < wx_vec.size() ; i++){
		...
		for(int j = 0 ; j < iter ; j++){
			time += rm_lin(model, N, M, max_dev, wx_vec[i], wx_vec[i]);
		}
		printf(" %6f \n\n", time/10);
	}

	// for(int i = 0 ; i < wx_vec.size() ; i++){
	// 	rm_lin(model, N, M, max_dev, wx_vec[i], wx_vec[i]);
	// }
